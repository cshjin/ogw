...
def auto_grad(x, funObj, options):
    ...
    Args:
        x (np.array): variable
        funObj (function handler): objective function
        options (dict): options

    Returns:
        (float, float): function value, gradient value

    Notes:
        the funObj should return a single value here - the objective (i.e., no gradient)
    """
    p = len(x)
    f = funObj(x)
