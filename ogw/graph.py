...
class Graph():
    ...
    def distance_matrix(self, method='shortest_path', changeInf=True, maxvaluemulti=10, force_recompute=False):
        ...
        if (self.C is None) or force_recompute:
            ...
            if method == 'laplacian':
                C = np.diag(A @ np.ones(A.shape[0])) - A

            if method == 'weighted_shortest_path':
                d = self.reshaper(
                    np.array([v for (k, v) in nx.get_node_attributes(self.nx_graph, 'attr_name').items()]))
                D = ot.dist(d, d)
...
def find_thresh(C, inf=0.5, sup=3, step=10, metric="sp"):
...
def draw_rel(G, draw=True, shiftx=0, shifty=0, return_pos=False,
             with_labels=True, swipy=False, swipx=False, vmin=0, vmax=7):
...
def draw_transp(G1, G2, transp, shiftx=1, shifty=0, thresh=0.09,
                swipy=False, swipx=False, vmin=0, vmax=7, with_labels=True):
    ...
    for k1, v1 in pos1.items():
        for k2, v2 in pos2.items():
            if (transp[invd1[k1], invd2[k2]] > thresh):
                plt.plot([pos1[k1][0], pos2[k2][0]], [pos1[k1][1], pos2[k2][1]], 'k--',
